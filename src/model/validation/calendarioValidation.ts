...
const turnoPersonasSchema = z.object({
  turno: z
    ...
    .refine(
      (val) =>
        Object.values(TipoTurno).includes(val.toLowerCase() as TipoTurno),
      {
        message:
          "El valor de 'turno' no es válido, debe ser 'cena' o 'almuerzo'.",
      }
    ),
  personas: z
    .string()
    ...
    .refine(
      ...
      {
        message:
          "El parámetro 'personas' debe ser un número mayor que 0 y no mayor a 10.",
      }
    ),
});
...
export const validateTurnoPersonas = (query: any) =>
  turnoPersonasSchema.safeParseAsync(query);

export const validateTurnoPersonasFecha = (query: any) =>
  turnoPersonasFechaSchema.safeParseAsync(query);
